...
export async function sendBillingReminders() {
  ...
  const { data: users, error: userErr } = await supabase
    .from("users")
    .select("id, name, email, is_pro, phone:phone_number")
    .in("id", userIds);
  ...
  for (const sub of subs) {
    ...
    if (user.is_pro && user.phone) {
      const reminderMessage = `Hi ${userName}, your ${sub.name} renews in 3 days for ${amountFormatted}. Log in at renwize.com to review it.`;
      ...
      try {
        ...
        failed.push({
          subscriptionId: sub.id,
          reason: `SMS: ${smsErr instanceof Error ? smsErr.message : String(smsErr)}`,
        });
      }

      try {
        await sendWhatsApp(user.phone, reminderMessage);
      } catch (waErr) {
        failed.push({
          subscriptionId: sub.id,
          reason: `WhatsApp: ${waErr instanceof Error ? waErr.message : String(waErr)}`,
        });
      }
    }
